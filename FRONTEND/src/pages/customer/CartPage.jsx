...
import { clearPendingCheckout } from "../../utils/checkoutStorage";
...
function CartPage() {
  ...
  // Extract a human-friendly error message from an Axios error so the user
  // sees WHY an offer failed (e.g. "Offer usage limit exceeded",
  // "Minimum purchase amount is 4000") instead of silently seeing nothing.
  ...
  // Apply an offer (product-specific or cart-level). When called with an

  // explicit `offerArg`, skips the offerCode lookup (used by the Apply buttons

  // inside the Available Offers list to avoid React state-update races).

  const applyOffer = async (offerArg) => {
    ...
    if (!offer) {
      ...
      if (!offer) {
        ...
        console.warn("No matching offer found for code:", offerCode);

        showToast(
          "warn",
          "Invalid Offer",
          `No matching offer found for "${offerCode}".`,
        );
    ...
    // Block apply when we already know the per-user usage quota is exhausted.
    // The backend would reject this with a 400 anyway; surfacing it client-side
    // keeps the behaviour consistent across all offer types.
    if (
      offer.usage_limit_per_user &&
      (offer.user_usage_count ?? 0) >= offer.usage_limit_per_user
    ) {
      showToast(
        "warn",
        "Offer Unavailable",
        `You've already used "${offer.offer_name}" the maximum number of times.`,
      );
      return;
    }
    ...
    try {
      ...
      if (offer.type === "product") {
        ...
        if (!cartItem) {
          ...
          console.warn("Cart item not found for product offer:", offer.offer_id);

          showToast(
            "warn",
            "Product Not In Cart",
            "Add the product to your cart before applying this offer.",
          );
      ...
      showToast(
        "success",
        "Offer Applied",
        `"${offer.offer_name}" applied to your cart.`,
      );
      ...
      showToast(
        "error",
        "Offer Not Applied",
        getApiErrorMessage(error, "Unable to apply this offer right now."),
      );
  ...
  const applyProductOffer = async (offer) => {
    ...
    try {
      ...
      showToast(
        "success",
        "Offer Applied",
        `"${offer.offer_name}" applied to this item.`,
      );
  ...
  const removeItemOffer = async (cartItemId, offerName) => {
    ...
    try {
      ...
      showToast(
        ...
        "Offer Removed",
        offerName
          ? `"${offerName}" removed from this item.`
          : "Item offer removed.",
      );
  ...
  const removeOffer = async () => {
    ...
    try {
      ...
      showToast("success", "Offer Removed", "Cart offer removed.");
                  ...
                  <div className="space-y-2">
                    ...
                    {getCartOffers().length > 0 ? (
                      ...
                      getCartOffers().map((offer) => {
                        ...
                        const isExhausted =
                          !!offer.usage_limit_per_user &&
                          (offer.user_usage_count ?? 0) >=
                            offer.usage_limit_per_user;

                        const isApplicable = meetsMinPurchase && !isExhausted;
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                {!meetsMinPurchase && offer.min_purchase_amount && (
                                ...
                                {isExhausted && (
                                  <p className="text-xs text-red-500 dark:text-red-400 mt-1">
                                    You've already used this offer the maximum
                                    number of times.
                                  </p>
                                )}
